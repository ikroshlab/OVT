...
public class  GeometryBuffer {
	...
	private final static int GROW_INDICES = 1;   // todo - changed from 64
	private final static int GROW_POINTS  = 1;   // todo - changed from 512
	...
	public int          indexPos;                  // The current index position
	public int          pointPos;                  // The current position in points array
	public GeometryType type;                      // The current geometry type
	public PointF       mTmpPoint    = new PointF();
	public int          pointLimit;                   // todo - depricated field???
	 ...
	 * @return    when out is null a temporary PointF is returned which belongs to GeometryBuffer.
	 */
	...
	public GeometryBuffer addPoint(float x, float y) {
		if (pointPos > pointLimit)	ensurePointSize((pointPos >> 1) + 1, true);
	...
	public GeometryBuffer startLine() {
		...
		if (index[indexPos] > 0) {
			...
			/* start next */
			if ((index[0] >= 0) && (++indexPos >= index.length))  ensureIndexSize(indexPos, true);
		...
		/* set new end marker */
		if (index.length > indexPos + 1)  index[indexPos + 1] = -1;
		return this;
	}
	...
	public GeometryBuffer startPolygon() {
		...
		if ((indexPos + 3) > index.length)	ensureIndexSize(indexPos + 2, true);
	...
	public void startHole() {
		...
		if ((indexPos + 2) > index.length)	ensureIndexSize(indexPos + 1, true);
	...
	public float[] ensurePointSize(int size, boolean copy) {
		if (size * 2 < points.length)  return points;

		size = size * 2 + GROW_POINTS;

		float[] newPoints = new float[size];
		if (copy)	System.arraycopy(points, 0, newPoints, 0, points.length);
	...
	public int[] ensureIndexSize(int size, boolean copy) {
		if (size < index.length) return index;

		int[] newIndex = new int[size + GROW_INDICES];
		if (copy)  System.arraycopy(index, 0, newIndex, 0, index.length);
	...
	private void setOrCheckMode(GeometryType m) {
		...
		if (type != GeometryType.NONE)  throw new IllegalArgumentException("not cleared " + m + "<>" + type);
	...
	private void checkMode(GeometryType m) {
		if (type != m)  throw new IllegalArgumentException("not cleared " + m + "<>" + type);
	}
	 ...
	 * 
	 * @param minSqDist min distance
	 * @param keepLines keep endpoint when line would otherwise collapse into a single point
	 */
	public void simplify(float minSqDist, boolean keepLines) {
		...
		for (int idx = 0; idx < index.length; idx++) {
			...
			for (int pt = 2, end = index[idx]; pt < end; pt += 2) {
				...
				if (Math.sqrt(dx*dx + dy*dy) < minSqDist) {  // todo SQRT added!
					if (!keepLines || (pt < end-2))	continue;
				}
				px = cx;
				py = cy;
				points[outPos++] = cx;
