...
public final class LineBucket extends RenderBucket {
	...
	public void addLine(GeometryBuffer geom) {
		if      (geom.isPoly())		addLine(geom.points, geom.index, -1, true);
		else if (geom.isLine())		addLine(geom.points, geom.index, -1, false);
		else logDebug("geometry must be LINE or POLYGON");
	}

	public void addLine(float[] points, int numPoints, boolean closed) {
		if (numPoints >= 4)  addLine(points, null, numPoints, closed);
	}
	...
	private void addLine(float[] points, int[] index, int numPoints, boolean closed) {
		...
		if      (line.cap == Cap.ROUND)		rounded = true;
		else if (line.cap == Cap.SQUARE)	squared = true;
		...
		for (int i = 0, pos = 0; i < n; i++) {
			...
			/* start an endpoint are equal */
			if (length == 4 && points[ipos] == points[ipos + 2] && points[ipos + 1] == points[ipos + 3])  continue;

			/* avoid simple 180 degree angles */
			if (length == 6 && points[ipos] == points[ipos + 4] && points[ipos + 1] == points[ipos + 5])  length -= 2;
	...
	public static final class Renderer {
		...
		private final static float COORD_SCALE_BY_DIR_SCALE = MapRenderer.COORD_SCALE / LineBucket.DIR_SCALE;
		...
		public static RenderBucket draw(RenderBucket b, GLViewport v,
			...
			gl.vertexAttribPointer(s.aPos, 4, GL.SHORT, false, 0, buckets.offset[LINE]);
