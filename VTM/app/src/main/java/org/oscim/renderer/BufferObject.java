...
public final class BufferObject extends Inlist<BufferObject> {
	...
	public static void checkBufferUsage(boolean force) {
		...
		if (mBufferMemoryUsage < LIMIT_BUFFERS)	return;

		logDebug("use: " + mBufferMemoryUsage / MB + "MB");
		mBufferMemoryUsage -= BufferObject.limitUsage(MB);
		logDebug("now: " + mBufferMemoryUsage / MB + "MB");
	}
	...
	public static synchronized BufferObject get(int target, int size) {
		...
		if (pool[t] == null) {
			if (counter[t] != 0) throw new IllegalStateException("lost objects: " + counter[t]);
		...
		if (size != 0) {
			...
			for (; bo != null; bo = bo.next) {
				if (bo.size > size) {
					if (min == null || min.next.size > bo.size) 	min = prev;
				}
				prev = bo;
			}
    ...
    // handling Logging:
    private static void logDebug(String msg) { Log.d("VTM", " ### " + "BufferObject" + " ###  " + msg); }
}
