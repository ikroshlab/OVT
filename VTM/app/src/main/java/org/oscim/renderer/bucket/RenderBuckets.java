...
public class RenderBuckets extends TileData {
	...
	private RenderBucket getBucket(int level, int type) {
		...
		if (mCurBucket != null && mCurBucket.level == level) {
			bucket = mCurBucket;
			if (bucket.type != type) {
				logDebug("BUG wrong bucket:  " + Integer.valueOf(bucket.type) + " " + Integer.valueOf(type) + "  on level: " + Integer.valueOf(level));
		...
		if (b == null || b.level > level) {
			...
			if (mCurBucket != null && level > mCurBucket.level) 	b = mCurBucket;
			...
			while (true) {
				...
				if (b.next == null || b.next.level > level) 	break;
		...
		if (bucket == null) {
			...
			if      (type == LINE)			bucket = new LineBucket(level);
			else if (type == POLYGON)		bucket = new PolygonBucket(level);
			else if (type == TEXLINE)		bucket = new LineTexBucket(level);
			else if (type == MESH)			bucket = new MeshBucket(level);
			else if (type == HAIRLINE)		bucket = new HairLineBucket(level);
	...
	public void setFrom(RenderBuckets buckets) {
		if (buckets == this)	throw new IllegalArgumentException("Cannot set from oneself!");
	...
	public boolean compile(boolean addFill) {
		...
		if (addFill) vboSize += 8;

		ShortBuffer vboData = MapRenderer.getShortBuffer(vboSize);

		if (addFill) vboData.put(fillCoords, 0, 8);
		...
		if (vboSize != vboData.position()) {
			logDebug("wrong vertex buffer size: "
                        + " new size: " + vboSize
		...
		if (iboSize > 0 && iboSize != iboData.position()) {
			logDebug("wrong indice buffer size: "
                        + " new size: " + iboSize
		...
		if (vbo == null)  vbo = BufferObject.get(GL.ARRAY_BUFFER, vboSize);

		vbo.loadBufferData(vboData.flip(), vboSize * 2);

		if (iboSize > 0) {
			if (ibo == null)  ibo = BufferObject.get(GL.ELEMENT_ARRAY_BUFFER, iboSize);
