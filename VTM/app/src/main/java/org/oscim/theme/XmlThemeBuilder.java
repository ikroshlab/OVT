...
public class XmlThemeBuilder extends DefaultHandler {
	...
	public static void logUnknownAttribute(String element, String name, String value, int attributeIndex) {
        logDebug("unknown attribute in element {} () : {} = {}" + element + "  " + attributeIndex + " " + name + " " + value);
	}
	...
	private final HashMap<String, RenderStyle> mStyles = new HashMap<String, RenderStyle>(10);

	private final HashMap<String, TextStyle.TextBuilder<?>> mTextStyles = new HashMap<String, TextStyle.TextBuilder<?>>(10);
	...
	public void startElement(String uri, String localName, String qName,
		...
		try {
			if (ELEMENT_NAME_RENDER_THEME.equals(localName)) {
				...
				checkState(localName, Element.RENDERING_INSTRUCTION);
				logDebug("unknown element: " + localName);
	...
	private RuleBuilder createRule(String localName, Attributes attributes, Stack<RuleBuilder> ruleStack) {
	...
	private TextureRegion getAtlasRegion(String src) {
		...
		if (texture == null)  logDebug("missing texture atlas item '" + src + "'");
	...
	private void handleLineElement(String localName, Attributes attributes, boolean isStyle)
		...
		if (use != null) {
			...
			if (style == null) {
				logDebug("missing line style 'use': " + use);
				return;
			}
		}
	 ...
	 */
	private LineStyle createLine(LineStyle line, String elementName, Attributes attributes, int level, boolean isOutline) {
	...
	private void handleAreaElement(String localName, Attributes attributes, boolean isStyle)
		...
		if (use != null) {
			...
			if (style == null) {
				logDebug("missing area style 'use': " + use);
				return;
			}
		}
	...
	private void addOutline(String style) {
		if (style != null) {
			...
			if (line != null && line.outline) mCurrentRule.addStyle(line);
			else                              logDebug("BUG not an outline style: " + style);
		}
	}
	...
	private void handleTextElement(String localName, Attributes attributes, boolean isStyle, boolean isCaption) throws SAXException {
		...
		if (style != null) {
			...
			if (pt == null) {
				logDebug("missing text style: " + style);
				return;
			}
		}
	 ...
	 */
	private TextBuilder<?> createText(String elementName, Attributes attributes, boolean caption, TextBuilder<?> style) {
		TextBuilder<?> b;
		if (style == null) {
	...
	public static void validateNonNegative(String name, float value) {
		if (value < 0) throw new ThemeException(name + " must not be negative: " + value);
	}

	public static void validateExists(String name, Object obj, String elementName) {
		if (obj == null) throw new ThemeException("missing attribute " + name + " for element: " + elementName);
	}



    // handling Logging:
    private static void logDebug(String msg) { Log.d("VTM", " ### " + "XmlThemeBuilder" + " ###  " + msg); }
}
