...
public abstract class PbfDecoder implements ITileDecoder {
	...
	protected final static ProtobufException INVALID_PACKED_SIZE = new ProtobufException("invalid message size");
	...
	protected int decodeVarint32() throws IOException {
		...
		for (int shift = 0; shift < 32; shift += 7) {
			if (bytesLeft == 0)  bytesLeft = fillBuffer(1);
	...
	protected String decodeString() throws IOException {
		...
		if (mStringDecoder == null)	result = new String(buffer, bufferPos, size, "UTF-8");
		else              			result = mStringDecoder.decode(buffer, bufferPos, size);
	...
	protected float decodeFloat() throws IOException {
		if (bufferPos + 4 > bufferFill)	fillBuffer(4);
	...
	protected int decodeInterleavedPoints(GeometryBuffer geom, float scale)  throws IOException {
		...
		if (pos != bufferPos + bytes) throw INVALID_PACKED_SIZE;
	...
	public void decodeVarintArray(int num, int[] array) throws IOException {
		...
		for (int end = pos + bytes; pos < end; cnt++) {
			if (cnt == num)  throw new ProtobufException("invalid array size " + num);
	...
	protected short[] decodeUnsignedVarintArray(short[] array) throws IOException {
		...
		if (arrayLength > cnt)	array[cnt] = -1;
	...
	public int fillBuffer(int size) throws IOException {
		...
		if (bytesLeft >= size) 	return bytesLeft;
		...
		if (size > maxSize) {
			...
			if (debug)	logDebug("shift " + bufferFill + " " + bufferPos + " " + size);
		...
		while ((bufferFill - bufferPos) < size) {
			...
			if (len < 0) {
				if (debug)	logDebug("finished reading " + mMsgPos);
	...
	protected static int readUnsignedInt(InputStream is, byte[] buf) throws IOException {
		...
		if (read < 4)  return read < 0 ? (read * 10) : read;
    ...
    // handling Logging:
    private static void logDebug(String msg) { Log.d("VTM", " ### " + "PbfDecoder" + " ###  " + msg); }
}
