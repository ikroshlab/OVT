...
class S3DBTileLoader extends TileLoader {
	...
	private void initTile(MapTile tile) {
		...
		mGroundScale = (float) MercatorProjection.groundResolution(lat, 1 << mTile.zoomLevel);
	...
	public void process(MapElement element) {
		...
		if (element.type != GeometryType.TRIS) {
			logDebug("wrong type " + element.type);
			return;
		}

		if (mParts == null)	initTile(mTile);

		boolean isRoof = element.tags.containsKey(ROOF_KEY);
		if (isRoof) logDebug(element.tags.toString());
