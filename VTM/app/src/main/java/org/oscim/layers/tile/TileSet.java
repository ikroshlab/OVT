 ...
 * tiles. Tiles will be locked and not be modified until getActiveTiles passes them
...
public final class TileSet {
	...
	public MapTile getTile(int x, int y) {
		for (int i = 0; i < cnt; i++)
			if (tiles[i].tileX == x && tiles[i].tileY == y)	return tiles[i];
	...
	public static class CoordComparator implements Comparator<MapTile> {
		...
		public int compare(MapTile lhs, MapTile rhs) {
			if (lhs.tileX == rhs.tileX) {
				if (lhs.tileY == rhs.tileY)	return 0;

				if (lhs.tileY <  rhs.tileY)	return 1;

				return -1;
			}
			if (lhs.tileX < rhs.tileX)   	return 1;
	...
	public boolean contains(MapTile t) {
		for (int i = 0; i < cnt; i++)
			if (tiles[i].equals(t))	return true;
