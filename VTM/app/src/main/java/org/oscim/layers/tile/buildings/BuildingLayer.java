...
public class BuildingLayer extends Layer implements TileLoaderThemeHook {
	...
	public BuildingLayer(Map map, VectorTileLayer tileLayer, int zoomMin, int zoomMax) {
		...
		mRenderer = new BuildingRenderer(tileLayer.tileRenderer(), zoomMin, zoomMax,false, TRANSLUCENT);
		if (POST_AA)  mRenderer = new OffscreenRenderer(Mode.SSAO_FXAA, mRenderer);
	}
	...
	public boolean render(MapTile tile, RenderBuckets buckets, MapElement element,
		...
		if (!(style instanceof ExtrusionStyle))	return false;
		...
		if (v != null)	height = Integer.parseInt(v);

		v = element.tags.getValue(Tag.KEY_MIN_HEIGHT);
		if (v != null)	minHeight = Integer.parseInt(v);

		/* 12m default */
		if (height == 0)  height = 12 * 100;
	...
	public void complete(MapTile tile, boolean success) {
		if (success) get(tile).prepare();
		else         get(tile).setBuckets(null);
	}
