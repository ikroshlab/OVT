...
public class TileManager {
	...
	private final TileIndex<TileNode, MapTile> mIndex = new TileIndex<TileNode, MapTile>() {
		        ...
		        public void removeItem(MapTile t) {
			        if (t.node == null)  throw new IllegalStateException("Already removed: " + t);
	...
	public void init() {
		if (mCurrentTiles != null)	mCurrentTiles.releaseTiles();
		...
		for (int i = 0; i < mTilesEnd; i++) {
			...
			// In case the tile is still loading or used by another thread:
			// clear when returned from loader or becomes unlocked
			t.setState(DEADBEEF);
		}
	...
	public boolean update(MapPosition pos) {
		...
		if (mZoomTable == null) {
			...
			//logDebug("p:{} {}:{}=>{} | {} <> {}" + mLoadParent + " " + mPrevZoomlevel + " " + pos.zoomLevel + " " + tileZoom + "  " + scaleDiv + "  " + (pos.scale / (1 << tileZoom)));
		} else {
			mLoadParent = false;
			int match = 0;
			for (int z : mZoomTable) {
				if (z <= tileZoom && z > match)  match = z;
			}
		...
		if (mCacheReduce < mCacheLimit / 2) {
			if (BufferObject.isMaxFill()) {
				mCacheReduce += 10;
				if (dbg) logDebug("reduce cache: " + (mCacheLimit - mCacheReduce));
			} else {
				mCacheReduce = 0;
			}
		}
	...
	private void addToCache(MapTile tile) {
		...
		if (mTilesEnd == mTiles.length) {
			...
			if (mTilesEnd == mTiles.length) {
				logDebug("realloc tiles: " + mTilesEnd);
	...
	private boolean removeFromCache(MapTile t) {
		...
		if (dbg) logDebug("remove from cache: " + t + "  " + t.state() + "  " + t.isLocked());
	...
	private void limitCache(MapPosition pos, int remove) {
		...
		if ((remove < CACHE_CLEAR_THRESHOLD) && (newTileCnt < MAX_TILES_IN_QUEUE))  return;
		...
		for (int i = mTilesCount - 1; i >= 0 && remove > 0; i--) {
			...
			/* don't remove tile used by TileRenderer, or somewhere else
			 * try again in next run. */
			if (t.isLocked()) {
				if (dbg) logDebug(" " + t + " locked " + " state=" +  t.state + " d=" + t.distance);
				continue;
			}
			...
			if (t.state(LOADING)) {
				t.setState(CANCEL);
				if (dbg) logDebug(" " + t + " canceled   d=" + t.distance);
				continue;
			}
			...
			if (t.state(NEW_DATA)) {
				newTileCnt--;
				if (dbg) logDebug(" " + t + " unused  d=" + t.distance);
			}

			if (!t.state(READY | NEW_DATA)) {
				logDebug("stuff that should be here! " + t + "   " + t.state());
			}
	...
	public void jobCompleted(MapTile tile, boolean success) {
		...
		/* locked means the tile is visible or referenced by a tile that might be visible. */
		if (tile != null && tile.isLocked()) mMap.render();
	}
	...
	class JobCompletedEvent implements Runnable {
		...
		public void run() {
			...
			logDebug("problem Loading tile: " + tile  + "  ...." + " state:" + tile.state());
	...
	private final ScanBox mScanBox = new ScanBox() {
		...
		protected void setVisible(int y, int x1, int x2) {
			...
			for (int x = x1; x < x2; x++) {
				...
				if (cnt == maxTiles) {
					logDebug("too many tiles: " + maxTiles);
					break;
				}
				int xx = x;
