...
public class MapTile extends Tile {
	 ...
	 * ElementLayers is always at first position (for VectorTileLayer).
     * TileLoaderHooks may add additional data. See e.g. {@link LabelTileLoaderHook}.
	 */
    ...
    public MapTile(TileNode node, int tileX, int tileY, int zoomLevel) {
        ...
        this.node = node;
        this.mTile = new Tile(tileX, tileY, (byte)zoomLevel);  // fiction
    }
	...
	void lock() {
		if (state == DEADBEEF) {
			logDebug("Locking dead tile " + this);
			return;
		}
	...
	void unlock() {
		...
		for (int i = 0; i < 4; i++) {
			if ((proxy & (1 << i)) != 0)  node.child(i).refs--;
		}
		...
		if (state == DEADBEEF) {
			logDebug("Unlock dead tile " + this);
			clear();
		}
	}
	...
	/**
	 * Get the default ElementLayers which are added by {@link VectorTileLoader}
	 */
	public RenderBuckets getBuckets() {
		if (!(data instanceof RenderBuckets)) return null;
	...
	public MapTile getProxyChild(int id, byte state) {
		if ((proxy & (1 << id)) == 0)	return null;

		MapTile child = node.child(id);
		if (child == null || (child.state & state) == 0)  return null;

		return child;
	}

	public MapTile getParent() {
		if ((proxy & PROXY_PARENT) == 0) return null;
	...
	public MapTile getProxy(int proxy, byte state) {
		...
		if ((this.proxy & proxy) == 0)	return null;
