...
public class VectorTileLoader extends TileLoader implements RenderStyle.Callback {
	...
	public boolean loadTile(MapTile tile) {
		...
		if (mTileDataSource == null) {
			logDebug(">>>>>> loadTile - no tile source is set");
			return false;
		}
		...
		if (renderTheme == null) {
			logDebug(">>>>>> loadTile - no theme is set");
			return false;
		}
		...
		try {
            ...
            //logDebug(">>>>>> loadTile - querying data source......");

			mTileDataSource.query(tile, this);
		} catch (NullPointerException e) {
			logDebug(">>>>>> loadTile - NPE :" + tile + "   " + e.getMessage());
			e.printStackTrace();
		} catch (Exception e) {
			logDebug(">>>>>> loadTile - Ex: " + tile + "   " + e.getMessage());
			e.printStackTrace();
			return false;
		}
		return true;
	...
	protected static int getValidLayer(int layer) {
		if      (layer < 0) 	   return 0;
		else if (layer >= LAYERS)  return LAYERS - 1;
		else 		               return layer;
	...
	public void setDataSource(ITileDataSource dataSource) {
		...
		if      (dataSource instanceof org.oscim.tiling.source.mapfile.MapDatabase) {
            mCache          = ((MapDatabase)mTileDataSource).getMapFileTileSource().getTileCache();
        }
        else if (dataSource instanceof org.oscim.tiling.source.UrlTileDataSource) {
            mCache          = ((UrlTileDataSource)mTileDataSource).getTileCache();
        }

        mUseCache = mCache != null;
	}
	...
	/**
	 * Override this method to change tags that should be passed to {@link RenderTheme} matching.
	...
	public void process(MapElement element) {
		if (isCanceled() || !mTile.state(LOADING)) return;

		if (mTileLayer.callProcessHooks(mTile, mBuckets, element))	return;
	...
	public void renderWay(LineStyle line, int level) {
		...
		if (line.stipple == 0) {
			if (line.outline && mCurLineBucket == null) {
				logDebug("missing line for outline! " + mElement.tags + " lvl:" + level + " layer:" + mElement.layer);
				return;
			}
	...
	public void renderArea(AreaStyle area, int level) {
		...
		if (mTile.zoomLevel < area.fadeScale)	return;
