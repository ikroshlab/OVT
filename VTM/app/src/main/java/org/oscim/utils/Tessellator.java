...
public class Tessellator {
	...
	public static int tessellate(float[] points, int ppos, int numPoints, int[] index,
		...
		if (numPoints != nverts) {
			logDebug("tess ----- skip poly: " + nverts + " " + numPoints);
			tess.dispose();
			return 0;
		}
		...
		for (int offset = 0; offset < nelems;) {
			...
			for (int i = start; i < end; i++) {
				if (indices[i] < 0) {
					logDebug(">>>> eeek   start:" + start + "  end:" + end + "    "   + Arrays.toString(Arrays.copyOfRange(indices, start, end)));
					break;
				}
				indices[i] *= 2;
			}
    ...
    // handling Logging:
    private static void logDebug(String msg) { Log.d("VTM", " ### " + "Tessellator" + " ###  " + msg); }
}
