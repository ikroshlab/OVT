...
import static ovt.DeCoder.coder;
import ovt.GeometryBuffer.GeometryType;

import static ovt.GeometryBuffer.GeometryType.LINE;
import static ovt.GeometryBuffer.GeometryType.POLY;
import ovt.header.MapFileInfo;
import ovt.header.SubFileParameter;
...
public class MapDatabase implements ITileDataSource {
    ...
    @Override
    public void query(Tile tile) {

        //logDebug("query data source...");
        ...
        double tXdouble = (double) tile.tileX / (1 << tile.zoomLevel);
        double tYdouble = (double) tile.tileY / (1 << tile.zoomLevel);
        
        minDeltaLat = (int) (Math.abs(MercatorProjection.toLatitude(tYdouble + size)
                                    - MercatorProjection.toLatitude(tYdouble)) * 1e6) / simplify;
        minDeltaLon = (int) (Math.abs(MercatorProjection.toLongitude(tXdouble + size)
                                    - MercatorProjection.toLongitude(tXdouble)) * 1e6) / simplify;
        ...
        try {
            processBlocks(queryParameters, subFileParameter);
        ...
        // save tile into DB    
        // 1. serialize tile into byte array:
        byte[] rawTile = encodeTile();

        // 2. store this tile into DB:
        if (rawTile != null) {
            logDebug(">>>>>> TILE -->  " + tX + "/" + tY + "/" + tZ + " - storing into DB...  size: " + rawTile.length);
            logDebug(">>>>>> TILE -->  " + tX + "/" + tY + "/" + tZ + " -        ways:   tags size: " + tags_total_bytes);
            logDebug(">>>>>> TILE -->  " + tX + "/" + tY + "/" + tZ + " -        ways: points size: " + points_total_bytes);
            logDebug(">>>>>> TILE -->  " + tX + "/" + tY + "/" + tZ + " -        ways:  index size: " + index_total_bytes);
	...
	/////////////////////////////////////  PROCESSING  /////////////////////////////////////////////
    private void processBlocks(QueryParameters queryParams, SubFileParameter subFileParameter) throws IOException {
        ...
        for (long row = queryParams.fromBlockY; row <= queryParams.toBlockY; row++) {
            for (long column = queryParams.fromBlockX; column <= queryParams.toBlockX; column++) {
                ...
                */
                ...
                processBlock(queryParams, subFileParameter);
            }
        }
    }
    ...
    private void processBlock(QueryParameters queryParameters, SubFileParameter subFileParameter) {
        ...
        // process POIs
        if (!processPOIs(poisOnQueryZoomLevel))  return;
        ...
        // process WAYs
        if (!processWays(queryParameters, waysOnQueryZoomLevel)) 	return;
     ...
     */
    private boolean processPOIs(int numberOfPois) {
        ...
        for (int elementCounter = numberOfPois; elementCounter != 0; --elementCounter) {
            ...
            mTileProjection.projectPoint(latitude, longitude, e);  // todo - is this needed for debugger?
     ...
     */
    private boolean processWays(QueryParameters queryParameters, int numberOfWays) {
        ...
        for (int elementCounter = numberOfWays; elementCounter != 0; --elementCounter) {
            ...
            for (int wayDataBlock = 0; wayDataBlock < wayDataBlocks; wayDataBlock++) {
                ...
                mTileProjection.project(e);  // todo - add to decoder?
            ...
            /*
            if      (tZ <= 8)             min_dist = 20.0f;
            else if (tZ >  8 && tZ <= 10) min_dist = 10.0f;
            else if (tZ > 10 && tZ <= 12) min_dist =  5.0f;
            else if (tZ > 12 && tZ <= 15) min_dist =  2.0f;
            else if (tZ > 15 && tZ <= 17) min_dist =  1.0f;
    ...
    // serialize tile into byte array
            ...
            for (int i=0; i<ways.size(); i++) {  // ways
                ...
                if (tZ<11) {                                                            
                    if (coder.get(tags[0].key)   != null) {
                        ...
                        tags_total_bytes += tags[0].key.getBytes().length;
                    }                    
                    if (coder.get(tags[0].value) != null)  {
                        ...
                        tags_total_bytes += tags[0].value.getBytes().length;
                    }                   
                }
                else {
                    for (Tag t : tags) {  
                ...
                points_total_bytes += pts.length * 4;
                ...
                index_total_bytes += idx.length * 2;
            }
    ...
    // handling Logging:
    void logDebug(String msg) { System.out.println(" ### " + getClass().getName() + " ###  " + msg); }
