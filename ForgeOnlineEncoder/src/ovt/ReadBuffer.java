...
public class ReadBuffer {
	...
	static final int               MAXIMUM_BUFFER_SIZE = 8000000;  // Maximum buffer size which is supported by this implementation.
	 ...
	 * If the capacity of the read buffer is too small, a larger one is created automatically.
	 * 
	...
	public boolean readFromFile(int length) throws IOException {
		...
		if (mBufferData == null || mBufferData.length < length) {   // ensure that the read buffer is large enough

			if (length > MAXIMUM_BUFFER_SIZE) {                     // ensure that the read buffer is not too large
				logDebug("invalid read length: " + length);
				return false;
			}
	 ...
	 * <p> The byte order is big-endian.
	 * 
	...
	public String readUTF8EncodedString(int stringLength) {
		...
		logDebug("invalid string length: " + stringLength);
		return null;
	}
